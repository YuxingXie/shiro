...
>> ### <a href='#cp3-4' id='ccp3-4'>身份认证流程</a> 
...
>>> #### <a href='#cp13-2-1' id='ccp13-2-1'>集成目标</a>
>>> #### <a href='#cp13-2-2' id='ccp13-2-2'>前置步骤</a>
>>> #### <a href='#cp13-2-3' id='ccp13-2-3'>整合shiro</a>
...
<b>SecurityManager</b>：相当于 SpringMVC 中的 DispatcherServlet 或者 Struts2 中的 FilterDispatcher；是 Shiro 的心脏；所有具体的交互都通过 SecurityManager 进行控制；
...
<b>Realm</b>：可以有 1 个或多个 Realm，可以认为是安全实体数据源，即用于获取安全实体的；可以是 JDBC 实现，也可以是 LDAP 实现，或者内存实现等等；由用户提供；注意：Shiro 
不知道你的用户 / 权限存储在哪及以何种格式存储；所以我们一般在应用中都需要实现自己的 Realm；

<b>SessionManager</b>：如果写过 Servlet 就应该知道 Session 的概念，Session 呢需要有人去管理它的生命周期，这个组件就是 SessionManager；而 Shiro 并不仅仅可以用在 Web 
环境，也可以用在如普通的 JavaSE 环境、EJB 等环境；所有呢，Shiro 就抽象了一个自己的
 Session 来管理主体与应用之间交互的数据；这样的话，比如我们在 Web 环境用，刚开始是一台 Web 服务器；接着又上了台 EJB 服务器；这时想把两台服务器的会话数据放到一个地方，
 这个时候就可以实现自己的分布式会话（如把数据放到 Memcached 服务器）；

<b>SessionDAO</b>：DAO 大家都用过，数据访问对象，用于会话的 CRUD，比如我们想把 Session 保存到数据库，那么可以实现自己的 SessionDAO，通过如 JDBC 写到数据库；
...
## 第二章：Shiro 身份验证 <a href='#ccp3'>目录</a>
...
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png) 注：可以理解为一种.ini形式的realm?
```ini
[users]
zhang=123
wang=123
...
* 如果身份验证失败请捕获 AuthenticationException 或其子类，常见的如： DisabledAccountException（禁用的帐号）、LockedAccountException（锁定的帐号）、
UnknownAccountException（错误的帐号）、ExcessiveAttemptsException（登录失败次数过多）、IncorrectCredentialsException （错误的凭证）、
...
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png) 注：我们看到shiro为我们实现了登录和退出的功能，
我们仔细思考，shiro背后到底做了哪些工作呢？首先登录的时候的用户名密码验证是shiro完成的，其次登录状态的保持也是shiro实现的，猜想应该
是用到了Session机制，另外还实现了退出功能。其中登录和退出显示调用login和logout方法，shiro自动保存和清除了会话状态。
...
注：这里看到了Realm接口使用的两个小配件：AuthenticationToken和AuthenticationInfo，有必要了解一下：
```java
...
```
它们分别代表身份验证时使用的令牌和验证信息，从API设计来看，一个AuthenticationToken持有一个Object类型的Principal以及
Object类型的Credentials，注意是复数，表明令牌可以持有多个证明。AuthenticationInfo因为表示用户登录后的信息，则需要持有用户的
所有身份(PrincipalCollection)和所有凭证(Object)。
...
```
 ![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)
注：上面的代码我们看到了AuthenticationInfo的具体实现SimpleAuthenticationInfo，以及AuthenticationToken的具体实现UsernamePasswordToken。
另外上面的代码还包括几个重要信息：
* 考虑应用场景，大部分程序可能会从数据库读取用户信息再和令牌对比，所以ini配置的Realm比较少见,需要自定义Realm;
* 在自定义的Realm中实现令牌和Realm对比，正确时返回AuthenticationInfo，否则抛出异常；
* 在IOC框架中，Realm应该可以作为bean注入数据访问层bean实现CRUD。
...
3、测试用例请参考
com.github.zhangkaitao.shiro.chapter2.LoginLogoutTest 的 testCustomRealm 测试方法，只需要把之前的 shiro.ini 
配置文件改成 shiro-realm.ini 即可。
 
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)
注：如果在ini中指定了\[roles\]和\[users\]，则该配置可以充当一个Realm，否则需要在ini中指定securityManager.realms。
...
```
securityManager 会按照 realms 指定的顺序进行身份认证。此处我们使用显示指定顺序的方式指定了 Realm 的顺序，如果删除 
“securityManager.realms=$myRealm1,$myRealm2”，那么securityManager 会按照 realm 声明的顺序进行使用
（即无需设置 realms 属性，其会自动发现），当我们显示指定 realm 后，其他没有指定 realm 将被忽略，如 
...
以后一般继承 AuthorizingRealm（授权）即可；其继承了 AuthenticatingRealm（即身份验证），而且也间接继承了 CachingRealm
（带有缓存实现）。其中主要默认实现如下：

<b>org.apache.shiro.realm.text.IniRealm</b>：\[users\] 部分指定用户名 / 密码及其角色；\[roles\] 部分指定角色即权限信息；

<b>org.apache.shiro.realm.text.PropertiesRealm</b>： user.username=password,role1,role2 指定用户名 / 密码及其角色；
role.role1=permission1,permission2 指定角色及权限信息；

<b>org.apache.shiro.realm.jdbc.JdbcRealm</b>：通过 sql 查询相应的信息，如 “select password from users where 
username = ?” 获取用户密码，“select password, password_salt from users where username = ?” 获取用户密码及盐；
...
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)注：AuthenticatingRealm抽象类中定义了2个抽象方法,
注意它们的返回类型，AuthenticationInfo 是我们见过的，它是Realm的一个配件：
```java
...
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png) 注：到此为止，所有的realm配置都依赖于ini配置，
如何进行编程式或注解式配置呢？我们暂且留着这个问题继续学习。
...
```
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)注：前面提到过AuthenticationInfo是Realm的配件，当我们
在Realm中自己实现验证成功后返回一个AuthenticationInfo实例（SimpleAuthenticationInfo），在这里我们看看如何通过Authenticator实现验证。
...
<B>AtLeastOneSuccessfulStrategy</B>：只要有一个 Realm 验证成功即可，和 FirstSuccessfulStrategy 不同，返回所有 Realm 
身份验证成功的认证信息；
...
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)
注：shiro默认的authenticator就是org.apache.shiro.authc.pam.ModularRealmAuthenticator,但这里为何要明文指出，是因为要设置
securityManager.authenticator及securityManager.authenticator.authenticationStrategy属性。另，后面的章节中配置authorizer也类似于authenticator。
...
```
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png) 注：我们看到匹配令牌和realm的具体代码仍然在Realm中。    
...
```
shiro-authenticator-all-fail.ini 与 shiro-authenticator-all-success.ini 不同的配置是使用了 
securityManager.realms=$myRealm1,$myRealm2；即 myRealm 验证失败。

对于 AtLeastOneSuccessfulStrategy 和 FirstSuccessfulStrategy 的区别，请参照 testAtLeastOneSuccessfulStrategyWithSuccess
和 testFirstOneSuccessfulStrategyWithSuccess 测试方法。唯一不同点一个是返回所有验证成功的 Realm 的认证信息；
另一个是只返回第一个验证成功的 Realm 的认证信息。
...
自定义实现时一般继承 org.apache.shiro.authc.pam.AbstractAuthenticationStrategy 即可，具体可以参考代码
com.github.zhangkaitao.shiro.chapter2.authenticator.strategy 包下 OnlyOneAuthenticatorStrategy 和 AtLeastTwoAuthenticatorStrategy。
...
打印文档等等。。。
如上可以看出，权限代表了用户有没有操作某个资源的权利，即反映在某个资源上的操作允不允许，不反映谁去执行这个操作。所以后续还需要把权限赋予给用户，
即定义哪个用户允许在某个资源上做什么操作（权限），Shiro 不会去做这件事情，而是由实现人员提供。
Shiro 支持粗粒度权限（如用户模块的所有权限）和细粒度权限（操作某个用户的权限，即实例级别的），后续部分介绍。
#### 角色
角色代表了操作集合，可以理解为权限的集合，一般情况下我们会赋予用户角色而不是权限，即这样用户可以拥有一组权限，赋予权限时比较方便。典型的如：项目经理、技术总监、CTO、
开发工程师等都是角色，不同的角色拥有一组不同的权限。
#### 隐式角色：
即直接通过角色来验证用户有没有操作权限，如在应用中 CTO、技术总监、开发工程师可以使用打印机，假设某天不允许开发工程师使用打印机，此时需要从应用中删除相应代码；再如在应用中 CTO、
技术总监可以查看用户、查看权限；突然有一天不允许技术总监查看用户、查看权限了，需要在相关代码中把技术总监角色从判断逻辑中删除掉；即粒度是以角色为单位进行访问控制的，粒度较粗；
如果进行修改可能造成多处代码修改。
#### 显示角色：
在程序中通过权限控制谁能访问某个资源，角色聚合一组权限集合；这样假设哪个角色不能访问某个资源，只需要从角色代表的权限集合中移除即可；无须修改多处代码；即粒度是以资源/实例为单位的；
粒度较细。
...
到此基于角色的访问控制（即隐式角色）就完成了，这种方式的缺点就是如果很多地方进行了角色判断，但是有一天不需要了那么就需要修改相应代码把所有相关的地方进行删除；
这就是粗粒度造成的问题。

![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)注：如果还不理解隠式角色的话，可以补充一句：隐式角色只定义了一个角色，但不定义角色可以干什么，
需要在程序代码中判断角色去干什么或不能干什么。其实是一种不完整的定义。
...
规则：“用户名=密码，角色 1，角色 2”“角色=权限 1，权限 2”，即首先根据用户名找到角色，然后根据角色再找到权限；即角色是权限集合；Shiro 同样不进行权限的维护，需要我们通过 
Realm 返回相应的权限信息。只需要维护“用户——角色”之间的关系即可。

![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)注：和隐式角色相比较，显示角色规定了一个角色可以干什么(即权限，也即可以对何种资源进行何种操作)。
...
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)
注：原文作者念念不忘的isPermittedAny，hasAnyRole完全可以遍历所有的role或permission用hasRole或 isPermitted,其中一个返回true则isPermittedAny，hasAnyRole
返回true实现。
...
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)注：无论是检查权限checkPermission，还是检查角色checkRole其方法主题都是Subject,
而不是role.checkPermission。
...
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)
注：总结授权的三要素即Subject、Role、Permission，其中Permission=Resource+Operation。那么问题来了，如何用字符串表示某个资源与某个操作呢？其实资源和操作都是
开发者定义的字符串，比如user资源，其实是开发者定义了一个字符串"user"表示某些资源，delete操作也是开发者定义的一个字符串"delete"表示某类操作，一般在数据库中以字符串存在。
...
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png) 注：这里所说的实例又是什么鬼？
...
用户拥有资源“system:user”的“update”操作权限。
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)注：如果没有理解我前面讲的"资源和操作都是开发者定义的字符串"，这里可能会被原作者搞糊涂，
按照原作者的规则定义，system:user难道不应该是表示system资源的user操作吗？但设想如果我们定义的表示资源的字符串就是system:user，就可以想通了。按照这个理解，原则上
字符串通配符中可以包含任意多个":"。
...
用户拥有资源“system:user”的“update”和“delete”操作权限。如上可以简写成：
...
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)注：到现在还没理解资源的实例代表什么。暂且理解为某个动态资源的ID吧。
...
![](https://github.com/YuxingXie/shiro/raw/master/doc/notice.png)注：在认证一章中，我们看到实际进行用户名/密码比对的工作是在Realm中完成的，权限检查也同样。
我们看看Authorizer是如何定义的：
```java
...
```
这么多的方法在AuthorizingRealm中都实现了。而这些方法在也可以通过Subject调用,实际上Subject是委托SecurityManager实现的。前面接触过的JdbcRealm也继承自AuthorizingRealm。
...
Authorizer 的职责是进行授权（访问控制），是 Shiro API 中授权核心的入口点，其提供了相应的角色/权限判断接口，具体请参考其 Javadoc。SecurityManager 继承了 Authorizer 
接口，且提供了 ModularRealmAuthorizer 用于多 Realm 时的授权匹配。PermissionResolver 用于解析权限字符串到 Permission 实例，而 RolePermissionResolver 
用于根据角色解析相应的权限集合。
...
## 第六章：Shiro Realm <a href='#ccp7'>目录</a>
...
* doGetAuthenticationInfo 获取身份验证相关信息：首先根据传入的用户名获取 User 信息；然后如果 user 为空，那么抛出没找到帐号异常 UnknownAccountException；如果 
user 找到但锁定了抛出锁定异常 LockedAccountException；最后生成 AuthenticationInfo 信息，交给间接父类 AuthenticatingRealm 使用 CredentialsMatcher 
进行判断密码是否匹配，如果不匹配将抛出密码错误异常 IncorrectCredentialsException；另外如果密码重试此处太多将抛出超出重试次数异常 ExcessiveAttemptsException；
在组装 SimpleAuthenticationInfo 信息时，需要传入：身份信息（用户名）、凭据（密文密码）、盐（username+salt），CredentialsMatcher 
...
Account 相当于我们之前的 User，SimpleAccount 是其一个实现；在 IniRealm、PropertiesRealm 这种静态创建帐号信息的场景中使用，这些 Realm 直接继承了 
SimpleAccountRealm，而 SimpleAccountRealm 提供了相关的 API 来动态维护 SimpleAccount；即可以通过这些 API 来动态增删改查 SimpleAccount；
动态增删改查角色 / 权限信息。及如果您的帐号不是特别多，可以使用这种方式，具体请参考 SimpleAccountRealm Javadoc。
...
RunAs 即实现 “允许 A 假设为 B 身份进行访问”；通过调用 subject.runAs(b) 进行访问；接着调用 subject.getPrincipals 将获取到 B 的身份；此时调用 isRunAs 将返回 
true；而 a 的身份需要通过 subject. getPreviousPrincipals 获取；如果不需要 RunAs 了调用 subject. releaseRunAs 即可。
...
实现线程之间的 Subject 传播，因为 Subject 是线程绑定的；因此在多线程执行中需要传播到相应的线程才能获取到相应的 Subject。最简单的办法就是通过 execute(runnable/callable
 实例) 直接调用；或者通过 associateWith(runnable/callable 实例) 得到一个包装后的实例；它们都是通过：1、把当前线程的 Subject 绑定过去；2、在线程执行结束后自动释放。
...
### 第七章：与传统Web集成 <a href='#ccp8-1'>目录</a>

Shiro 提供了与 Web 集成的支持，其通过一个 ShiroFilter 入口来拦截需要安全控制的 URL，然后进行相应的控制，ShiroFilter 类似于如 Strut2/SpringMVC 这种 web 
框架的前端控制器，其是安全控制的入口点，其负责读取配置（如 ini 配置文件），然后判断 URL 是否需要登录 / 权限等工作。
...
##### 1、创建 webapp 应用
此处我们使用了 jetty-maven-plugin 和 tomcat7-maven-plugin 插件；这样可以直接使用 “mvn jetty:run” 或“mvn tomcat7:run”直接运行 webapp 了。然后通过 URL
...
DelegatingFilterProxy 作用是自动到 spring 容器查找名字为 shiroFilter（filter-name）的 bean 并把所有 Filter 的操作委托给它。然后将 ShiroFilter 配置到 
spring 容器即可：
...
```
1. doGet 请求时展示登录页面；
2. doPost 时进行登录，登录时收集 username/password 参数，然后提交给 Subject 进行登录。如果有错误再返回到登录页面；否则跳转到登录成功页面
（此处应该返回到访问登录页面之前的那个页面，或者没有上一个页面时访问主页）。
3. JSP 页面请参考源码。
...
1. authcBasic 是 org.apache.shiro.web.filter.authc.BasicHttpAuthenticationFilter 类型的实例，其用于实现基于 Basic 的身份验证；applicationName 
用于弹出的登录框显示信息使用，如图：
...
Shiro 也提供了 logout 拦截器用于退出，其是 org.apache.shiro.web.filter.authc.LogoutFilter 类型的实例，我们可以在 shiro.ini 
配置文件中通过如下配置完成退出：
```ini
[main]
...
OncePerRequestFilter 用于防止多次执行 Filter 的；也就是说一次请求只会走一次拦截器链；另外提供 enabled 属性，表示是否开启该拦截器实例，默认 enabled=true 表示开启，
如果不想让某个拦截器工作，可以设置为 false 即可。
...
如果要注册自定义拦截器，IniSecurityManagerFactory/WebIniSecurityManagerFactory 在启动时会自动扫描 ini 配置文件中的 \[filters\]/
\[main\] 部分并注册这些拦截器到 
...
* 如果没有登录，看看是否是登录请求，如果是 get 方法的登录页面请求，则继续拦截器链（到请求页面），否则如果是 get 方法的其他页面请求则保存当前请求
并重定向到登录页面；
...
此处可以通过继承 AuthenticatingFilter 实现，其提供了很多登录相关的基础代码。另外可以参考 Shiro 内嵌的 FormAuthenticationFilter 的源码，
思路是一样的。
...
此处可以继承 AuthorizationFilter 实现，其提供了授权相关的基础代码。另外可以参考 Shiro 内嵌的 RolesAuthorizationFilter 的源码，只是实现
 hasAllRoles 逻辑。
...
<td>
<p>基于表单的拦截器；如 “`/**=authc`”，如果没有登录会跳到相应的登录页面登录；主要属性：usernameParam：表单提交的用户名参数名（ username）；
  passwordParam：表单提交的密码参数名（password）； rememberMeParam：表单提交的密码参数名（rememberMe）；  loginUrl：登录页面地址
 （/login.jsp）；successUrl：登录成功后的默认重定向地址； failureKeyAttribute：登录失败后错误信息存储 key（shiroLoginFailure）；</p>
</td>
</tr><tr><td>
<p>authcBasic</p>
</td>
...
<td>
<p>角色授权拦截器，验证用户是否拥有所有角色；主要属性： loginUrl：登录页面地址（/login.jsp）；unauthorizedUrl：未授权后重定向的地址；示例
 “/admin/**=roles[admin]”</p>
</td>
</tr><tr><td>
<p>perms</p>
...
<td>
<p>端口拦截器，主要属性：port（80）：可以通过的端口；示例 “/test= port[80]”，如果用户访问该页面是非 80，将自动将请求端口改为 80 并重定向到该
 80 端口，其他路径 / 参数等都一样</p>
</td>
</tr><tr><td>
<p>rest</p>
...
<td>
<p>rest 风格拦截器，自动根据请求方法构建权限字符串（GET=read, POST=create,PUT=update,DELETE=delete,HEAD=read,TRACE=read,
OPTIONS=read, MKCOL=create）构建权限字符串；示例 “/users=rest[user]”，会自动拼出“user:read,user:create,user:update,user:delete”
 权限字符串进行权限匹配（所有都得匹配，isPermittedAll）；</p>
</td>
</tr><tr><td>
<p>ssl</p>
</td>
...
<td>
<p>不创建会话拦截器，调用 subject.getSession(false) 不会有什么问题，但是如果 subject.getSession(true) 将抛出 
DisabledSessionException 异常；</p>
</td>
</tr></tbody></table>

另外还提供了一个 org.apache.shiro.web.filter.authz.HostFilter，即主机拦截器，比如其提供了属性：authorizedIps：已授权的 ip 地址，
deniedIps：表示拒绝的 ip 地址；
不过目前还没有完全实现，不可用。
...
所谓会话，即用户访问应用时保持的连接关系，在多次交互中应用能够识别出当前访问的用户是谁，且可以在多次交互中保存一些数据。如访问一些网站时登录成功后，
网站可以记住用户，
且在退出之前都可以识别当前用户是谁。
...
获取会话的启动时间及最后访问时间；如果是 JavaSE 应用需要自己定期调用 session.touch() 去更新最后访问时间；如果是 Web 应用，每次进入 
ShiroFilter 都会自动调用 
...
```java
//如DefaultSessionManager在创建完session后会调用该方法；如保存到关系数据库/文件系统/NoSQL数据库；即可以实现会话的持久化；返回会话ID；
...
* securityManager.cacheManager：设置 SecurityManager 的 cacheManager，会自动设置实现了 CacheManagerAware 接口的相应对象，如 
SessionDAO 的 cacheManager；
...
如上会话验证调度器实现都是直接调用 AbstractValidatingSessionManager 的 validateSessions 方法进行验证，其直接调用 SessionDAO 的 
getActiveSessions 方法获取所有会话进行验证，如果会话比较多，会影响性能；可以考虑如分页获取会话并进行验证，
...
如果是在获取会话时验证了会话已过期，将抛出 InvalidSessionException；因此需要捕获这个异常并跳转到相应的页面告诉用户会话已过期，让其重新登录，
如可以在 web.xml 配置相应的错误页面：
```xml
<error-page>
...
#### 前置步骤 <a href='#ccp13-2-2'>目录</a>


##### 1、新建gradle项目

虽然原则上开发不应该依赖于特定工具，但为方便讲解，本小节使用集成开发环境intellij IDEA，操作系统macOS。
...
在Project或Module根目录中找到build.gradle，拷贝如下内容：
...
```
是不是比maven配置简单得多呢？书写简化，自动最新version，等等。

具体的gradle构建的技术细节本人也未深入研究，所以就先依样画葫芦吧。

##### 3、从.xml到java code配置

###### 程序入口

spring-boot-web与传统web相比，简直天翻地覆。首先WebRoot--WEB-INF--web.xml这种结构不见了，甚至连web容器都不见了。它现在就是一个jar程序。在终端使用

    java -jar xxxxx.jar
启动。首先我们要为程序构造一个入口。

com.lingyun.demos.shiro.config.Application:
...
没错，这是spring-boot的最小配置，如此简单一个web程序可以启动了，启动方法如下：
![](https://github.com/YuxingXie/shiro/raw/master/doc/sh-5.png)

甚至在该文件右键菜单点击 Run 'Application'即可启动。

浏览器地址栏输入 http://localhost:8080 访问，可以看到 Whitelabel Error Page 页，因为没有设置网站根路由，所以会返回一个错误页。我们在/src/main/resources
下新建一个public目录，新建一个index.html,重启一下程序，发现根路由可以访问了。

因为本小节不是gradle和spring-boot教学，所以简单入门就讲到这里。下面正式开讲shiro和spring-boot集成。


###### 集成spring-data-jpa

根据集成目标需要集成spring-data-jpa，我们在Application加上类注解
    
    @EnableJpaRepositories(basePackages = {"com.lingyun.demos.**.repository"})
    
@EnableJpaRepositories注解会让程序会自动检测包中的jpa repository。

再加上

    @EntityScan(basePackages ={"com.lingyun.demos.**.entity"})

@EntityScan注解用于检测jpa实体类。

现在我们建立几个实体类用于映射关系型数据库表字段。当然我们也可以先建表再建实体类。因为jpa框架有从实体类自动生成建表DDL并执行
的能力，我倾向于从实体类依靠框架生成数据库表结构。如果您喜欢先建表再把表映射为实体，我推荐使用google的一个脚手架工具code-generator。

###### 编写Entity

编写三张表sys_users，sys_roles，sys_permissions的实体映射。为什么不需要映射sys_user_role和sys_role_permission稍后说明。

com.lingyun.demos.shiro.entity.User:
...
```
com.lingyun.demos.shiro.entity.Role:
```java
@Entity
...
```
com.lingyun.demos.shiro.entity.Permission:
```java
@Entity
...
为了能够让jpa自动映射建表，我们需要新建一个在resources资源文件夹下的application.properties文件。这似乎与零配置的的构建目标违背，但我们的目标是让xml配置消失，
这并没有违背。application.properties文件如下：
```properties
spring.jpa.show-sql=true
spring.jpa.hibernate.ddl-auto=create-drop
spring.jpa.hibernate.dialect=org.hibernate.dialect.H2Dialect
```
spring-boot默认应用读取此文件中的配置。我们还可以配合@Profile注解，以便在不同的环境(开发环境，生产环境)建立
application-dev.properties,application-prod.properties应用不同的配置。

注：hibernate是jpa的一个具体实现，所以jpa配置中出现了hibernate的身影，我认为这是一个历史遗留问题。
...
到此，我们启动spring-boot程序后，程序会自动建表了，下面是控制台的部分打印：
```text
Hibernate: drop table sys_permissions if exists
Hibernate: drop table sys_role_permission if exists
Hibernate: drop table sys_roles if exists
Hibernate: drop table sys_user_role if exists
Hibernate: drop table sys_users if exists
Hibernate: create table sys_permissions (id varchar(255) not null, available boolean, description varchar(255), permission varchar(255), primary key (id))
Hibernate: create table sys_role_permission (role_id varchar(255) not null, permission_id varchar(255) not null)
Hibernate: create table sys_roles (id varchar(255) not null, available boolean, description varchar(255), role varchar(255), primary key (id))
Hibernate: create table sys_user_role (user_id varchar(255) not null, role_id varchar(255) not null, primary key (user_id, role_id))
Hibernate: create table sys_users (id varchar(255) not null, locked boolean, password varchar(255), salt varchar(255), username varchar(255), primary key (id))
Hibernate: alter table sys_role_permission add constraint UK_8n7kjpptsuop8jvkeh4ls34ct unique (permission_id)
Hibernate: alter table sys_role_permission add constraint FKnnsgqepyiht19by484jaugwkq foreign key (permission_id) references sys_permissions
Hibernate: alter table sys_role_permission add constraint FKncm6su7kk6wkk3n94u1txphrm foreign key (role_id) references sys_roles
Hibernate: alter table sys_user_role add constraint FK57npuokwjk6p6v3tbfxp3d65i foreign key (role_id) references sys_roles
Hibernate: alter table sys_user_role add constraint FKe2itwtwrl60nromvh4kqr5y97 foreign key (user_id) references sys_users
```

可以看到，我们并未创建SYS_USER_ROLE和SYS_ROLE_PERMISSION表映射实体，但通过@OneToMany和@ManyToMany注解映射生成了这两张表。

###### 构建service、repository和controller

Application的注解@SpringBootApplication()改为
```java
...
spring-boot会自动检测花括号内的这些包，如果包内包含@Controller和@Service的类会自动注册为spring Bean。

新建com.lingyun.demos.shiro.repository.UserRepository接口如下：

```java
public interface UserRepository extends JpaRepository<User,String>{
}
```
不需要任何注解，spring-data-jpa自动识别该接口为数据访问层接口，自动实现接口中的一些简单CRUD方法。如果要实现复杂的自定义方法，我们只需2步：

1. 建一个该接口名+Impl后缀为类名的类，编写一个自定义方法，类似于:
```java
...
```
注意UserRepositoryImpl不需要implements UserRepository。

2. 在UserRepository中定义该方法
```java
public interface UserRepository extends JpaRepository<User,String>{
    User findByUsername(String username);
}
```
接下来我们在控制器中调用该方法。为了简单，本人省略了service层编写，controller直接调用repository方法。

现在编写控制器
com.lingyun.demos.shiro.controller.UserController
```java
@Controller
@RequestMapping("/user")
...
```
新建一个spring-boot-web-shiro-integration/src/main/resources/public/user/index.html
```html
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
welcome to user index !
</body>
</html>
```
启动spring-boot程序，浏览器中输入 http://localhost:8080/user ，可以看到控制台打印和浏览器的响应都如我们的预期。

控制台打印：
```text
Hibernate: select user0_.id as id1_4_, user0_.locked as locked2_4_, user0_.password as password3_4_, user0_.salt as salt4_4_, user0_.username as username5_4_ from sys_users user0_ where user0_.username=?
no user find
```

浏览器显示：welcome to user index !

<div id='cp13-2-3'></div>

#### 整合shiro <a href='#ccp13-2-3'>目录</a>

现在只需要把shiro和spring-boot进行整合了。为了达成整合目标，我们不能使用xml和ini配置，也不能使用jsp标签。

首先我们编写一个AuthorizingRealm扩展类MyShiroRealm:
```java
...
然后我们要把配置移到java类中，我们编写一个ShiroConfiguration类：
```java
@Configuration
...
```
为了使配置生效，需要在Application.java的@SpringBootApplication注解的scanBasePackages加上ShiroConfiguration.java的包名：
```java
